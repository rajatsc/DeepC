...
matrix matmul(matrix a, matrix b)
{
    ...
    matrix b_tr=transpose_matrix(b);
    // TODO: 1.4 - Implement matrix multiplication. Make sure it's fast!

    int i,j,k;
    for (i=0;i < a.rows; i++){
        for (j=0;j < b_tr.rows; j++){
            float tmp=0;
            for (k=0; k < a.cols; k++){
                tmp=tmp+a.data[i*a.cols+k]*b_tr.data[j*a.cols+k];    
            }
            c.data[i*c.cols+j]=tmp;
        }
    }
    //yo
    free_matrix(b_tr);
...
float mean_matrix(matrix m)
{
    float mymean=0;
    int i;
    for(i=0; i < m.rows*m.cols; i++){
        mymean=m.data[i]+mymean;
    }

    mymean = mymean/(m.rows*m.cols);
    return mymean;
}